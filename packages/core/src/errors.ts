...
const table: Readonly<Record<string, readonly [ErrorFamily, string, string]>> = {
  ...
  invalid_timeout: ["connector",
    "A GitHub or Gmail connector's timeoutMs is outside 1 to 60000 milliseconds.",
  ...
  invalid_file_account: ["file",
    "The file connector's account id is longer than 256 characters or contains control characters.",
  ...
  invalid_file_roots: ["file",
    "The allowed roots list is empty or contains a path that is not absolute, or maxBytes is not a positive integer. Over MCP, the file connector was enabled without RECEIPTS_FILE_ROOTS.",
    "Configure allowed roots as absolute paths, or set RECEIPTS_FILE_ROOTS to absolute paths separated by the platform path delimiter (':' on macOS and Linux, ';' on Windows)."],
  file_too_large: ["file",
    "The file at the requested path is larger than the connector's maxBytes read limit (16 MiB by default).",
    "Raise maxBytes when constructing the connector if files this large are expected. No verification was issued; never repeat the write."],
  // ---- gmail -----------------------------------------------------------------
  invalid_gmail_payload: ["gmail",
    "canonicalGmailPayload received no To recipient, or a subject, body, cc, bcc, or html value that is not a string.",
    "Pass the approved To list, subject, and plain body as strings; cc, bcc, and the HTML body are optional strings. Recipient order, case, and display names do not affect the digest."],
