...
const table: Readonly<Record<string, readonly [ErrorFamily, string, string]>> = {
  ...
  duplicate_attempt: ["input",
    "An execution attempt was recorded, or a reservation was dispatched or released, for an account and action that already has a possible or observed write (including a write made without dispatch and then observed).",
    "Reconcile the existing write by reading the destination back and binding it. Never start a second write for the same approved action; a reservation that outlives its write is spent, not reusable."],
  ...
  active_claim: ["admission",
    "DUPLICATE reason: another live (unexpired, unreleased) reservation holds this account and action. Nothing has been written yet.",
    "Wait for that owner to dispatch, release, or expire, then claim again if the write is still wanted. Do not write, do not observe a destination that does not exist, and do not mint a new actionId to work around the reservation."],
  ...
  invalid_public_key: ["proof",
    "renderReceiptBadge received a trustedPublicKey that is not an Ed25519 public key (PEM or KeyObject). verifySignedReceipt folds the same condition into its generic invalid_proof result.",
