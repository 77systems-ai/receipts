...
## Claim from a staged file

Author the approved content into a staging file, then claim those exact bytes for the destination with `stageFilePayload(source, destination, { roots })`. It reads the staged file under the same rules as a read-back (absolute paths inside the roots, a regular UTF-8 file within the size limit) and returns `filePayload(destination, content)`. After dispatch, write `payload.content`, or copy the unchanged staged file, to the destination. The claimed bytes and the written bytes then cannot diverge; re-authoring content between approval and write is what turns a correct write into `package_unverified`. The source and destination must differ, and the destination may sit in a directory that does not exist yet. Over MCP, `receipts.prepare` with `file` does the same in one call.

```ts
const payload = stageFilePayload('/var/receipts/staging/brief.md', '/var/receipts/brief.md', { roots: ['/var/receipts'] });
await receipts.execute({ surface: 'file-write', ...identity, payload, execute: ({ payload }) => writeFile('/var/receipts/brief.md', payload.content) });
```
