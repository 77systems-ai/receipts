...
The repository's 0.4.0 evaluations for the [GitHub](../../docs/evaluations/github-0.4.0.json), [file](../../docs/evaluations/file-0.4.0.json), and [Gmail](../../docs/evaluations/gmail-0.4.0.json) connectors are generated by `npm run evaluate:connectors`, which runs this benchmark against each actual connector with its provider transport (or filesystem) controlled by the fixture. Earlier versions' evaluations stay in `docs/evaluations/` as historical artifacts. The separate [GitHub live example](../../examples/github-issues) verifies lost-response recovery against the real API and saves inspectable destination receipts locally.
